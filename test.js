...
tape('simple update', function (t) {
  bootstrap(function (port, node) {
    ...
    a.on('update:echo', function (data, callback) {
      ...
      t.same(data.value, Buffer.from('Greetings, Martian!'), 'expected data')
    ...
    a.ready(function () {
      var data = {
        command: 'echo',
        target: a.id,
        value: Buffer.from('Greetings, Martian!')
      }
      ...
      b.update(data, function (err, responses) {
        ...
        t.same(responses[0].value, Buffer.from('Greetings, Martian!'), 'echoed data')
        t.end()
      })
    })
  })
...
tape('simple query', function (t) {
  bootstrap(function (port, node) {
    ...
    a.on('query:greetings', function (data, callback) {
      t.same(data.value, null, 'expected data')
      callback(null, Buffer.from('martian'))
    })
    ...
    a.ready(function () {
      ...
      b.query(data, function (err, responses) {
        ...
        t.error(err, 'no errors')
        t.same(responses.length, 1, 'one response')
        t.same(responses[0].value, Buffer.from('martian'), 'responded')
        t.end()
      })
    })
  })
...
tape('swarm query', function (t) {
  bootstrap(function (port, node) {
    ...
    function done () {
      ...
      me.update({command: 'kv', target: key, value: Buffer.from('greetings')}, function (err, responses) {
        t.error(err, 'no error')
